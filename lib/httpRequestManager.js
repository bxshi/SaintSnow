...
p.getFinalStatus = function(){
    ...
    for(var i = 0; i<this.clientList.length; i++){
        ...
        transfer += tmpStatus.transfer;
    }
    var totalTime = this.stopTime-this.startTime;
    return {
        'Total Time' : totalTime,
        'Total Trans': success+error,
        'Total Transfered Data' : transfer/1024,
        'Success Trans' : success,
        'Error Trans' : error,
        'Success Rate' : success/(success+error) * 100,
        'Throughput' : success / Math.floor(totalTime/1000),
        'Data Throughput' : transfer / Math.floor(totalTime/1000) / 1024
        };
